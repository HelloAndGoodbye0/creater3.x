...
import { LobbyLanSpriteConfig } from "../../../../../../assets/Base/Config/BaseGameUIConfig";
...
export class LanguageSprite extends Component {
    ...
    private async updateSprite() {
        ...
        let nowLan = LanguageData.current
        ...
        {
            if(!oops.language.hasBundle(bundleName))
            {
                bundleName = `${this.gameID}_${LanguageManager.DEFAULT_LANGUAGE}`
            }
            this.loadSpriteFrame(bundleName,path,(err,sf:SpriteFrame)=>{
                if(err)
                {
                    //尝试加载默认图片
                    var defaultPath =  `content/sprite/lan/${LanguageManager.DEFAULT_LANGUAGE}/${this._spriteName}/spriteFrame`
                    bundleName = `${this.gameID}_${LanguageManager.DEFAULT_LANGUAGE}`
    ...
    private loadSpriteFrame(bundleName:string,path: string, callback: (err: Error | null, sf: SpriteFrame | null) => void) {
        oops.res.load(bundleName,path,SpriteFrame,(err,sf:SpriteFrame)=>{
           callback(err,sf)
        })
    }
    /**
