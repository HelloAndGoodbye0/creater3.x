...
type ExecuterFunc = (callback: CallbackObject, buffer: any) => void;
...
export class NetNode {
     ...
     * 是否准备好开始连接
     */
    ready4Connected():boolean
    {
        return this._state == NetNodeState.Closed
    }
    ...
    protected onChecked() {
        ...
        if (requests.length > 0) {
            ...
            for (var i = 0; i < requests.length;) {
                let req = requests[i];
                Logger.logNet(`send req ${req.rspCmd}`)
    ...
    request(reqProtocol: IRequestProtocol, rspObject: CallbackObject, showTips: boolean = true, force: boolean = false) {
        var rspCmd = this._protocolHelper!.handlerRequestPackage(reqProtocol).toString()
    ...
    protected base_request(reqProtocol: any, rspCmd: string, rspObject: CallbackObject, showTips: boolean = true, force: boolean = false) {
