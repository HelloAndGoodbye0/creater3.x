...
export class LayerManager {
     ...
     * @returns 
     */
    addConfig(UID:number,config: UIConfig): void {
        if(this.configs[UID])
        {
            console.warn("addConfig: UID is exist:",UID);
            return;
        }
        this.configs[UID] = config;
    }
    /**
     * @description: 添加node到指定层
     * @param {Node} node 药添加的节点
     * @param {layerType} layerType 层级
     * @return {*}
     */
    addNode(node:Node,layerType:LayerType)
    {
        switch (layerType) {
            case LayerType.UI:
            ...
            case LayerType.PopUp:
                this.popup.addNode(node);
                break;
            case LayerType.Dialog:
                this.dialog.addNode(node);
                break;
            case LayerType.System:
                this.system.addNode(node);
                break;
            case LayerType.Guide:
                this.guide.addChild(node);
                break;
        }
    }
    /**
