...
const SimpleParse_1 = require("./SimpleParse");
const CustomParse_1 = require("./CustomParse");
const ParticleParse_1 = require("./ParticleParse");
class PrefabParse {
    constructor() {
        /**
         * @zh 本次寻找过程中已经需找到了的uuid会存储在这里，而不会再次寻找
         */
        this._uuidSeekedMap = new Map();
        /**
         *
         */
        ...
        this._simpleParse = new SimpleParse_1.SimpleParse();
        this._customParse = new CustomParse_1.CustomParse();
        this._particleParse = new ParticleParse_1.ParticleParse();
    ...
    async testPrefab(prefabPath) {
        ...
        for (let nodeJson of fatherJson) {
            ...
            if (type != 'cc.Node')
                continue;
            // console.log('node Name: ', nodeJson._name);
            ...
            for (let each of components) {
                if (each && each.__id__) {
                    ...
                    let ret = await parse.parse(comJson, this._uuidSeekedMap, fatherJson);
                    if (ret.length) {
